...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initFlightAutocomplete() {
        ...
        function showCityModal(inputType) {
            ...
            currentTargetInput = inputType === 'origin' ? originInput : destinationInput;
            currentSearchType = inputType === 'origin' ? 'flights_origin' : 'flights_destination';
            
            // Update modal title based on input type
            modalTitle.textContent = inputType === 'origin' ? 'Selecciona ciudad de origen' : 'Selecciona ciudad de destino';
            ...
            // Handle modal close with back button
            modalBack.addEventListener('click', () => {
            ...
            modalInput.addEventListener('input', async function() {
                ...
                timeoutId = setTimeout(async () => {
                    const results = inputType === 'origin' ? await searchFlightOrigins(query) : await searchFlightDestinations(query);
        ...
        [originInput, destinationInput].forEach(input => {
            ...
            input.addEventListener('click', function(e) {
                ...
                // Determine input type and show modal
                const inputType = this.id === 'flight-origin' ? 'origin' : 'destination';
                showCityModal(inputType);
            });
                ...
                timeoutId = setTimeout(async () => {
                    ...
                    if (allItems.length > 0) {
                        allItems.forEach(item => {
                            ...
                            resultItem.addEventListener('click', () => {
                                console.log('🏨 Hotel seleccionado:', item);
                                ...
                                // Close modal
                                console.log('🔒 Cerrando modal de hotel...');
                                cityModal.style.display = 'none';
                                modalInput.value = '';
                                modalResults.innerHTML = '';
                                ...
                                setTimeout(() => {
                                    if (cityModal.parentNode) {
                                        cityModal.remove();
                                        console.log('✅ Modal de hotel removido del DOM');
                                    }
                                }, 300);
                            });
